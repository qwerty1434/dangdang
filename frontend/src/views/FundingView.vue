<template>
  <div>
    ...
    <div>
      ...
      <div class="mainTitle" v-if='this.id=="endedAt"'>마감임박순 펀딩리스트</div>
    ...
    <div class="total" v-if='this.id!="plan"'>{{funding.nowPrice}}원(코인)</div>
    <div class="startDay" v-if='this.id=="plan"'>시작일 : {{funding.startDate}}</div>
    <div class="remain" v-if='this.id!="plan"'>{{funding.remainDays}}일 남음</div>
    </div>
    </div>

    </div>
    <div class="numposition">
      <div class="pageNum" style="display:flex; justify-content: space-between; width: 200px; margin: auto; margin-top: 70px;">
...
export default {
  ...
  methods: {
    getFundings(){
      ...
      const categoryurl = "https://j7a306.p.ssafy.io/api/funding/search/category"
      if(this.id == "endedAt"){
        axios.get(dayurl, {
        params: {
          page : this.nowPage,
