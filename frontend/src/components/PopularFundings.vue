<template>
  <div>
    ...
    <div id="fundingList">
      ...
      <div v-for="funding in popularfundings" :key="funding.id" style="margin-right:30px" >
      <div class="thumbnail">
      <img
        :src="funding.img"
...
export default {
  ...
  methods: {
    getFundings(){
      this.checknext(this.popularnowPage+1);
      const url = "http://localhost:8080/api/funding/search"
      axios.get(url, {
        params: {
          page : this.popularnowPage,
          size : 4,
          type : "popular"
        }
    })
      .then(({data}) => {
        this.popularfundings = data.fundingList;
        console.log(data)
      }).catch((err)=> {
    ...
    left(){
      this.popularnowPage = this.popularnowPage -1;
      if(this.popularnowPage <0){
        this.popularnowPage = 0;
      }
      console.log(this.popularnowPage);
      this.getFundings()
    },
    right(){
      this.popularnowPage = this.popularnowPage + 1;
      if(this.popularfundings.length == 4){
        if(this.popularnextfundings.length != 0){
            this.getFundings()
        }else{
          this.popularnowPage = this.popularnowPage-1;
        }
    ...
    checknext(next){
      ...
      .then(({data}) => {
        this.popularnextfundings = data.fundingList;
        console.log(data)
      }).catch((err)=> {
